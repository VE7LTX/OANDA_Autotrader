...
def build_orders_client(config: AppConfig) -> OrdersAPI:
    """
    Create an OrdersAPI client for a resolved account configuration.
    """
...
def load_orders_client(
    accounts_path: str, group_name: str, account_name: str
) -> OrdersAPI:
    """
    Find an account by group name + account name and return an OrdersAPI client.
    """
    ...
    group, entry = select_account(groups, group_name, account_name)
    resolved = resolve_account_credentials(group, entry)
    return build_orders_client(resolved)
