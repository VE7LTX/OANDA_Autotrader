...
def _legacy_token_env(environment: str) -> str:
    return "FXPRACTICE_APIKEY" if environment == "practice" else "FXTRADE_APIKEY"


def _legacy_account_id_env(environment: str) -> str:
    return (
        "FXPRACTICE_ACCOUNT_ID" if environment == "practice" else "FXTRADE_ACCOUNT_ID"
    )
...
def _read_first_present_env(
    *var_names: str, required: bool, required_name: str | None = None
) -> str | None:
    for var_name in var_names:
        ...
        if value:
            return value.strip()
    if required:
        name = required_name or " or ".join(var_names)
...
def load_account_groups_or_default(path: str = "accounts.yaml") -> dict[str, AccountGroup]:
    """
    Load accounts from YAML when present, otherwise synthesize a minimal registry
    from the current environment variables.
    """

    if os.path.exists(path):
        return load_account_groups(path)

    _load_env_file()
    groups: dict[str, AccountGroup] = {}

    practice_account_id = _read_first_present_env(
        "DEMO_OANDA_ACCOUNT_ID",
        _legacy_account_id_env("practice"),
        required=False,
    )
    if practice_account_id:
        groups["demo"] = AccountGroup(
            key="demo",
            environment="practice",
            currency=os.getenv("OANDA_PRACTICE_CURRENCY", "CAD"),
            accounts=[
                AccountEntry(
                    name=os.getenv("OANDA_PRACTICE_ACCOUNT_NAME", "Primary"),
                    type="primary",
    ...
    live_account_id = _read_first_present_env(
        "LIVE_OANDA_ACCOUNT_ID",
        _legacy_account_id_env("live"),
        required=False,
    )
    if live_account_id:
        groups["live"] = AccountGroup(
            key="live",
            environment="live",
            currency=os.getenv("OANDA_LIVE_CURRENCY", "CAD"),
            accounts=[
                AccountEntry(
                    name=os.getenv("OANDA_LIVE_ACCOUNT_NAME", "Primary"),
                    type="primary",
    ...
    if not groups:
        raise ValueError(
            "accounts.yaml not found and no OANDA account IDs were discovered in the environment."
        )
...
def resolve_account_credentials(
    ...
    token = _read_first_present_env(
        _default_token_env(group.environment),
        _legacy_token_env(group.environment),
        required=True,
        required_name=f"{_default_token_env(group.environment)} or {_legacy_token_env(group.environment)}",
    )

    # Base URL override is optional; defaults to official endpoints.
    base_url = _read_first_present_env(
        "OANDA_API_BASE_PRACTICE" if group.environment == "practice" else "OANDA_API_BASE_LIVE",
        "FXPRACTICE_API_URL" if group.environment == "practice" else "FXTRADE_API_URL",
        required=False,
    )
    if not base_url:
    ...
    stream_base_url = _read_first_present_env(
        "OANDA_STREAM_BASE_PRACTICE"
        if group.environment == "practice"
        else "OANDA_STREAM_BASE_LIVE",
        "FXPRACTICE_STREAM_URL"
        if group.environment == "practice"
        else "FXTRADE_STREAM_URL",
        required=False,
    )
